...
- 玩家进服时，插件自动反查该角色名的绑定状态：已绑定玩家给出绑定提示；未绑定玩家静默跳过（避免每次进服打扰）
...
| 未绑定       | （无提示，静默跳过，避免每次进服打扰未绑定玩家）                                               |
| 查询失败     | （无提示，静默降级，仅服务端日志记录）                                                         |

> 注：「已绑定未核验」在新流程中不可达（绑定成功即已核验），仅存量旧数据可能出现。
...
# /v 命令冷却（秒）：默认 6 秒，低于后端 12 次/分钟限流上限；0 = 关闭（此时完全依赖后端 429 + resetAt 兜底）
command-cooldown-seconds: 6
```
...
| `connect-timeout-seconds`  | HTTP 连接超时（秒），默认 5，下限 1                                           |
| `request-timeout-seconds`  | HTTP 请求超时（秒），默认 10，下限 1；核验与反查请求共用                      |
| `command-cooldown-seconds` | /v 本地冷却（秒），默认 6；0 = 关闭（此时完全依赖后端 429 + resetAt 兜底）    |
...
| 玩家核验提示「玩家不一致」            | 网站提交的角色名与发起绑定时输入的名字不一致（**区分大小写**，如网站填错、代绑）；回网站重新发起 |
...
- [ ] 进服反查：已绑定 / 未核验玩家进服收到正确提示，未绑定玩家无打扰
